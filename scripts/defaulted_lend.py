...
def cli(cli_ctx, network):
    ...
    tx = pawn.claimDefaulted(pawn_id, sender=lender, gas_limit=100000000, gas_price=0, max_fee="0 gwei", max_priority_fee="0 gwei")
    logs = list(tx.decode_logs(pawn.PawnDefaultClaimed))
    assert len(logs) == 1
    print(f"Pawn with id {logs[0].pawnId} defaulted")
    pawn_data = pawn.idToData(pawn_id)
    print(f"Pawn data: {pawn_data}")
    print(f"Owner of nft with id {pawn_data.terms.nftId} is {nft.ownerOf(pawn_data.terms.nftId)}")
    print(f"borrower funds: {token.balanceOf(borrower)}")
    print(f"lender funds: {token.balanceOf(lender)}")
    print(f"returning nft to original owner...")
    nft.transferFrom(lender, borrower, pawn_data.terms.nftId, sender=lender, gas_limit=100000000, gas_price=0, max_fee="0 gwei", max_priority_fee="0 gwei")
    print(f"Owner of nft with id {pawn_data.terms.nftId} is {nft.ownerOf(pawn_data.terms.nftId)}")
